...
export const loadJson = async (causes) => {
  ...
  for (const [year, data] of Object.entries(yearlyData)) {
    ...
    for (const feature of data.features) {
      ...
      updatedData.features.push({
        ...feature,
        properties: { YEAR, MONTH, DAY, CAUSE, ...causeInfo },
      });
    }
  }
...
const main = async () => {
  const causes = await csvtojson()
    .fromFile("data/org/causes.csv")
    .then((csvRow) => {
      return csvRow;
    });

  await loadJson(causes);
};
